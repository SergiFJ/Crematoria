  ...
  Dir bfs_enemy_pioneer(Pos& x, Pos& enemy){
    ...
    while (not Q.empty() and moves != 0){
      ...
      for (int i = 0; i < 8; ++i){
        ...
        if (c.type != Rock){
          if (pos_ok(aux.first) and matriu[aux.first.i][aux.first.j] != 'R' and matriu[aux.first.i][aux.first.j] != 'H'){
            if (c.id != -1 and (unit(c.id).type == Hellhound or (unit(c.id).type == Furyan and unit(c.id).player != me() and moves > 18))){ //mirar quin numero de moves es millor
              enemy = aux.first;
              return aux.second; 
            }
            Q.push(aux);
