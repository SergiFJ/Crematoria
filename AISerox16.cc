//distancia que detecten furyans enemics
...
/////////////ES POT POSAR ELS DOS BFS EN UN FENT QUE LA CONDICIO DEL IF INTERN CRIDI UNA ALTRE FUNCIO//////////////////////
  ...
  Dir bfs_enemy_furyans(Pos& x, Pos& enemy, int meu){
  ...
  Dir bfs_run_furyans(Pos& x, Dir& d, int meu, Pos& enemy){
    ...
    if (compare_i == compare_j){    //puntes
        if (distancia.i < 0 and distancia.j < 0) final = TL;
         ...
         cerr << endl << endl << "Igual jo" << endl;
    }
    else {
        if (compare_i > compare_j){ //dalt o baix
            if (distancia.i > 0) final = Bottom;
            else final = Top;
             cerr << "Adalt jo" << distancia.i << distancia.j << endl;
        }
        else {
  ...
  Dir bfs_enemy_pioneer(Pos& x, Pos& enemy){
    vector<vector<char>> matriu(40,vector<char>(80));
    matriu[x.i][x.j] = 'H';
    queue<pair<Pos,Dir>> Q;
    Q.push(make_pair(x,None));
    ...
    while (not Q.empty() and moves != 0){
      ...
      for (int i = 0; i < 8; ++i){
        ...
        if (c.type != Rock){
          if (pos_ok(aux.first) and matriu[aux.first.i][aux.first.j] != 'R' and matriu[aux.first.i][aux.first.j] != 'H'){
            if (c.id != -1 and (unit(c.id).type == Hellhound or (unit(c.id).type == Furyan and unit(c.id).player != me() and moves > 16))){
              enemy = aux.first;
              return aux.second; 
            }
            Q.push(aux);
  ...
  void move_furyans(){    
    ...
    for (int i = 0; i < n; ++i){
      ...
      Unit F = unit(my_F[i]);
      Pos enemy;
      Dir m = bfs_enemy_furyans(F.pos,enemy, F.id);
      if (m == None){
    ...
    for (int i = 0; i < n; ++i){
      ...
      if (P.pos.k == 0){
        ...
        if (cell(P.pos).type == Elevator and P.pos.k == 0 and not daylight(aux))
            ...
            Pos enemy;
            Dir m = bfs_enemy_pioneer(P.pos,enemy);
            if (m == None){
