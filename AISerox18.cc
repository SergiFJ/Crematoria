//distancia que detecten furyans enemics i vida canviant el 15
...
/////////////ES POT POSAR ELS DOS BFS EN UN FENT QUE LA CONDICIO DEL IF INTERN CRIDI UNA ALTRE FUNCIO//////////////////////
  ...
  Dir bfs_enemy_furyans(Pos& x, int meu){
      ...
      for (int i = 0; i < 8; ++i){
        ...
        if (c.type != Rock){
          if (pos_ok(aux.first) and matriu[aux.first.i][aux.first.j] != 'R' and matriu[aux.first.i][aux.first.j] != 'H'){
            if (first){
              if (c.id != -1 and (unit(c.id).type == Hellhound or (unit(c.id).type == Furyan and unit(c.id).health > unit(meu).health)) and unit(c.id).player != me())
                return aux.second;
            }
            else if (c.id != -1 and unit(c.id).type == Hellhound)
                return aux.second;
          }
        }
        else {
  ...
  Dir bfs_run_furyans(Pos& x, Dir& d, int meu){
    vector<vector<char>> matriu(40,vector<char>(80));
    matriu[x.i][x.j] = 'F';
    queue<pair<Pos,Dir>> Q;
    Q.push(make_pair(x,None));
  ...
  Dir bfs_enemy_pioneer(Pos& x){
    ...
    while (not Q.empty() and moves != 0){
      ...
      for (int i = 0; i < 8; ++i){
        ...
        if (c.type != Rock){
          if (pos_ok(aux.first) and matriu[aux.first.i][aux.first.j] != 'R' and matriu[aux.first.i][aux.first.j] != 'H'){
            if (c.id != -1 and (unit(c.id).type == Hellhound or (unit(c.id).type == Furyan and unit(c.id).player != me() and moves > 16))) //mirar quin numero de moves es millor
                return aux.second;
            Q.push(aux);
  ...
  void move_furyans(){    
    ...
    for (int i = 0; i < n; ++i){
      vector<vector<char>> matriu(40,vector<char>(80));       
      Unit F = unit(my_F[i]);
      Dir m = bfs_enemy_furyans(F.pos,F.id);
      if (m == None){
        command(F.id, bfs_furyans(F.pos,F.id));
      }
      else {
        command(F.id, bfs_run_furyans(F.pos, m, F.id));
      }
    }
  }
