//lo de la vida del furyan ns si renta el igual en enemics
...
/////////////ES POT POSAR ELS DOS BFS EN UN FENT QUE LA CONDICIO DEL IF INTERN CRIDI UNA ALTRE FUNCIO//////////////////////
  ...
  Dir bfs_gem(Pos& x, int& id, VI& pioneers_gemes){
    vector<vector<char>> matriu(40,vector<char>(80));
    queue<pair<Pos,Dir>> Q;
    Q.push(make_pair(x,None));
    pair<Pos,Dir> aux;
    ...
    while (not Q.empty()){
      ...
      for (int i = 0; i < 8; ++i){
        ...
        if (pos_ok(aux.first) and matriu[aux.first.i][aux.first.j] != 'G' and not daylight(aux.first)){
          if (cell(aux.first).id != -1 and unit(cell(aux.first).id).player == me()){
            id = cell(aux.first).id;
            int n = pioneers_gemes.size();
            for (int j = 0; j < n; ++j){
                if (pioneers_gemes[j] == id){
                    pioneers_gemes.erase(pioneers_gemes.begin()+j);
                    return aux.second;
                }
            }
          }
  ...
  void move_pioneers(){
    ...
    for (int i = 0; i < n; ++i){
      ...
      if (P.pos.k == 0){
        ...
        pioneers_gemes.push_back(my_P[i]);
      }
    }

    //ANAR A PER GEMES
    vector<Pos> gemes = gems();
    int g = gemes.size();
    for (int i = 0; i < g; ++i){//Per cada gema
        int id;
        Dir m = bfs_gem(gemes[i],id,pioneers_gemes);
        if (m != None){
            int i = int(m) + 4;
            if (i > 7) i -= 7;
            command(id,Dir(i));
    ...
    n = pioneers_gemes.size();
    for (int i = 0; i < n; ++i){
        Unit P = unit(pioneers_gemes[i]);
