    ...
    for (int i = 0; i < n; ++i){
      ...
      if (P.pos.k == 0){
        ...
        if (cell(P.pos).type == Elevator and P.pos.k == 0 and not daylight(aux)){
            Pos sol = aux;
            Pos gema;
            Dir m = bfs_gem(aux,gema);
            int x = abs(aux.i-gema.i);
            int y = abs(aux.j-gema.j);
            if (x > 40) x -= 80;
            if (y > 40) y -= 80;

            int dist = max(abs(x), abs(y));
            cerr << dist << endl;
            while (not daylight(sol)){
                sol.j--;
            }
            if (sol.j > aux.j) sol.j -= 80;
            if (m != None and ((dist-1) <= (abs(aux.j-sol.j)/2))){
                command(P.id,Up);
            }
            else              
        ...
        Pos sol = P.pos;
        Pos gema;
        Dir m = bfs_gem(P.pos,gema);
        int x = abs(P.pos.i-gema.i);
        int y = abs(P.pos.j-gema.j);
        if (x > 40) x -= 80;
        if (y > 40) y -= 80;
        ...
        if (sol.j > P.pos.j) sol.j -= 80;
        if (m == None or ((dist-1) > (abs(P.pos.j-sol.j)/2))){
