//sha fet que els furyans nomes corrin dels hellhounds i arreglat errors varis
#include "Player.hh"
...
/////////////ES POT POSAR ELS DOS BFS EN UN FENT QUE LA CONDICIO DEL IF INTERN CRIDI UNA ALTRE FUNCIO//////////////////////
  ...
  Dir bfs_enemy_furyans(Pos& x, vector<vector<char>>& matriu){
  ...
  Dir bfs_run_furyans(Pos& x, Dir& d, VI& enemy, vector<vector<char>>& matriu){
    queue<pair<Pos,Dir>> Q;
    Q.push(make_pair(x,None));
    pair<Pos,Dir> aux;
    bool first = true;
    while (not Q.empty()){
      ...
      for (int i = 0; i < 8; ++i){
        Pos old_pos = aux.first;
        if (d != Dir(0) and d != Dir(7) and (i < int(d)-1 or i > int(d)+1)){
          Dir direction = Dir(i);
          if (first) aux.second = direction;
          aux.first += direction; 
        ...
        else if (d != Dir(0) and (i > 1 or i != 7)){
          Dir direction = Dir(i);
          if (first) aux.second = direction;
          aux.first += direction; 
          if (cell(aux.first).type != Rock){
  ...
  Dir bfs_enemy_pioneer(Pos& x, vector<vector<char>>& matriu){
    queue<pair<Pos,Dir>> Q;
    Q.push(make_pair(x,None));
    pair<Pos,Dir> aux;
    bool first = true;
  ...
  Dir bfs_run_pioneers(Pos& x, Dir& d, vector<vector<char>>& matriu){
    queue<pair<Pos,Dir>> Q;
    Q.push(make_pair(x,None));
    pair<Pos,Dir> aux;
    bool first = true;
  ...
  void move_furyans(){
    ...
    for (int i = 0; i < n; ++i){
      ...
      Unit F = unit(my_F[i]);
      Dir m = bfs_enemy_furyans(F.pos,matriu);
      if (m == None){
        command(F.id, bfs_furyans(F.pos,enemy_P,matriu));
      }
      else {
        command(F.id, bfs_run_furyans(F.pos, m, enemy_P, matriu));
      }
    }
  }
  ...
  void move_pioneers(){
    ...
    for (int i = 0; i < n; ++i){
      vector<vector<char>> matriu(40,vector<char>(80));
      Unit P = unit(my_P[i]);
      Dir m = bfs_enemy_pioneer(P.pos,matriu);
      if (m == None){
        command(P.id, bfs_pioneers(P.pos, matriu));
      }
      else {
        command(P.id, bfs_run_pioneers(P.pos, m, matriu));
      }
    }
  }
